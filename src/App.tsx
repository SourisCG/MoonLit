...
import { Clapperboard, Gamepad2, Moon, Settings } from "lucide-react";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    listen<HotkeyEvent>("moonlit://clip-hotkey", (event) => {
      setPresses((c) => c + 1);
      const when = new Date(Number(event.payload.pressed_at)).toLocaleTimeString();
      setLastPress(Number.isNaN(Date.parse(when)) ? new Date().toLocaleTimeString() : when);
    })
      .then((fn) => {
        unlisten = fn;
      })
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-moonlit-void font-sans text-slate-100 selection:bg-cyan-500/30">
      <MoonlitStarfield />
      <div className="pointer-events-none fixed left-1/2 top-0 h-[250px] w-[700px] -translate-x-1/2 bg-gradient-to-b from-cyan-500/10 via-indigo-500/5 to-transparent blur-3xl" />
      ...
      <div className="relative z-10 flex h-screen gap-4 p-4">
        <aside className="flex w-64 flex-col justify-between rounded-2xl border border-white/5 bg-moonlit-panel/50 p-4 shadow-2xl backdrop-blur-xl">
          <div>
            <div className="mb-6 flex items-center gap-2.5 px-2 py-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-tr from-cyan-400 to-indigo-500 shadow-[0_0_15px_rgba(56,189,248,0.4)]">
                <Moon size={16} className="text-black" strokeWidth={2.75} />
              </div>
        ...
        <main className="flex-1 overflow-y-auto rounded-2xl border border-white/5 bg-moonlit-panel/30 p-6 shadow-2xl backdrop-blur-xl">
