...
{
	QString line = QStringLiteral("%1 range=%2..%3 durationMs=%4")
			       .arg(result.succeeded ? QStringLiteral("ok")
						      : result.cancelled ? QStringLiteral("cancelled")
									 : QStringLiteral("failed"),
				   QString::number(request.startMs), QString::number(request.endMs),
				   QString::number(result.durationMs));
	if (!result.error.isEmpty()) {
		line += QStringLiteral(" error=") + result.error;
	}
	ffmpeg::appendExportLog(request.destinationPath, line);
}
						     ...
						     CancelCallback shouldCancel) const
{
	...
	for (AudioEncodeStream &audio : audioStreams) {
		const int drainResult = audio.drain(output.context, gain);
		if (drainResult < 0) {
